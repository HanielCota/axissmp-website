...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    if (!post) {
        return {
            title: "Notícia não encontrada | AxisSMP",
        };
    }
...
export default async function NewsArticlePage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    return (
        <main className="bg-background selection:bg-brand-orange/30 relative min-h-screen transition-colors duration-300">
            <Navbar />
